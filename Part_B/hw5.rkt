...
(define (racketlist->mupllist racklist)
  (cond [(null? racklist) (aunit)]
        [(list? (car racklist))
         (apair (racketlist->mupllist (car racklist))
                (racketlist->mupllist (cdr racklist)))]
        [#t (apair (car racklist)
                   (racketlist->mupllist (cdr racklist)))]))

(define (mupllist->racketlist mupllist)
  (cond [(aunit? mupllist) null]
        [(apair? (apair-e1 mupllist)) (cons (mupllist->racketlist (apair-e1 mupllist))
  ...
  (cond [(var? e) 
        ...
        [(ifgreater? e)
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (cond [(or (not (int? v1))
                      (not (int? v2)))
                  (error "ifgreater called with non-number")]
        ...
        [(mlet? e)
         (let ([v1 (eval-under-env (mlet-e e) env)])
           (eval-under-env (mlet-body e)
                           (append env (list (cons (mlet-var e) v1)))))]
        [(call? e)
               ...
               (if (not (closure? v1))
                   ...
                   (eval-under-env (fun-body (closure-fun v1))
                                   (append (closure-env v1)
                                           (if (false? (fun-nameopt (closure-fun v1))) ;; map func-name to closure
                                                               null
        ...
        [(fst? e)
         (let ([v1 (eval-under-env (fst-e e) env)])
           (if (apair? v1)
               (apair-e1 v1)
               (error "cannot call fst on non-pair")))]
         [(snd? e)
          (let ([v1 (eval-under-env (snd-e e) env)])
            (if (apair? v1)
                (apair-e2 v1)
                (error "cannot call snd on non-pair")))]
         [(isaunit? e)
